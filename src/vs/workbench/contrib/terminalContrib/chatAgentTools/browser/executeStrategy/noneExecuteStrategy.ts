	...
	async execute(commandLine: string, token: CancellationToken): Promise<ITerminalExecuteStrategyResult> {
		...
		try {
			...
			if (this._hasReceivedUserInput()) {
				...
				await waitForIdleWithPromptHeuristics(this._instance.onData, this._instance, 100);
			}
			...
			this._log('Waiting for idle with propnt heuristics');
			const promptResult = await waitForIdleWithPromptHeuristics(this._instance.onData, this._instace, 1000, 10000);
